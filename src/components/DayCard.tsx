...
export default function DayCard({ day, isToday, onMovementChange, onThrowChange, onGymChange }: Props) {
  const [open, setOpen] = useState(isToday)

  const throwType = day.throwing?.type
  const throwPillLabel =
    throwType === 'javelin_longtoss' ? 'Jav+LT' :
    throwType === 'mound_bullpen' ? 'Mound' : null
    ...
    <div className={`day-card${isToday ? ' today' : ''}${open ? ' open' : ''}`}>
      ...
      {open && (
        <div className="day-body">
          <MovementSection movement={day.movement} onChange={onMovementChange} />
